  ...
  if (eventError || !event) {
    return (
      <div className="flex flex-col min-h-screen items-center justify-center bg-gray-50 p-4 text-center">
        ...
        <Link href={`/${username}`} className="rounded-full bg-[#007AFF] px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-600 transition">
  ...
  if (!event.is_active) {
    return (
      <div className="flex flex-col min-h-screen items-center justify-center bg-gray-50 p-4 text-center">
        ...
        <p className="text-gray-600 mb-6">This event is currently not available for booking.</p>
        <Link href={`/${username}`} className="rounded-full bg-[#007AFF] px-6 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-600 transition">
          Back to {profile.user_name}'s page
        </Link>
      </div>
